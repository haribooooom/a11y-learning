...
export const questions: Question[] = [
  ...
  // ── 実例から学ぶ（SP版実測データ編） ────────────────────────
  {
    ...
    type: "multiple_choice",
    question:
      "あるサービスサイトのSP版を計測したところ、インタラクティブ要素393個のうち287個（73%）がタッチターゲットサイズ44px未満で、30個は20px未満だった。WCAG 2.5.5が推奨するタッチターゲットの最小サイズはいくつですか？",
    choices: [
      "24×24px",
      "32×32px",
      "44×44px",
    ...
    answer: 2,
    explanation:
      "WCAG 2.5.5（レベルAAA）およびApple HIG・Material Designは44×44px以上を推奨しています。WCAG 2.2で追加された2.5.8（レベルAA）では24×24pxが最低基準です。20px未満は指での操作がほぼ不可能で、手の震えや運動障害があるユーザーにとって特に深刻です（WCAG 2.5.5、2.5.8）。",
  },
  {
    id: "rw-27",
    themeId: "real-world",
    type: "multiple_choice",
    question:
      "ボトムナビゲーションの現在ページ（ホーム）が視覚的には青色で強調されているが、スクリーンリーダーで確認すると現在地が分からなかった。何が欠けていますか？",
    choices: [
      "active クラスの CSS",
      "aria-current=\"page\" の設定",
      "aria-selected=\"true\" の設定",
      "focus スタイルの設定",
    ],
    answer: 1,
    explanation:
      "視覚的な強調（色・太字）はスクリーンリーダーに伝わりません。現在ページを示すには aria-current=\"page\" をリンクに付与します。タブUIの場合は aria-selected=\"true\" を使います。これにより「現在のページ、ホーム、リンク」のように読み上げられます（WCAG 1.3.1）。",
  },
  {
    id: "rw-28",
    themeId: "real-world",
    type: "multiple_choice",
    question:
      "ページ内にヘッダーナビ・ボトムナビ・フッターナビと複数の `<nav>` 要素があるが、どれにも `aria-label` が設定されていない。スクリーンリーダーでランドマーク一覧を開くとどうなりますか？",
    choices: [
      "ナビゲーション要素が非表示になる",
      "すべて「ナビゲーション」と表示され、どれがどのナビか区別できない",
      "ナビゲーションが1つにまとめられる",
      "エラーが発生してページが読めなくなる",
    ],
    answer: 1,
    explanation:
      "複数の <nav> がある場合、aria-label で名前を付けないとスクリーンリーダーのランドマーク一覧に「ナビゲーション」「ナビゲーション」「ナビゲーション」と並びます。aria-label=\"メインナビゲーション\"・aria-label=\"フッターナビゲーション\" のように区別することで目的のナビへ直接ジャンプできます（WCAG 2.4.1、ARIA Landmark規則）。",
  },
  {
    id: "rw-29",
    themeId: "real-world",
    type: "multiple_choice",
    question:
      "ボトムナビの非アクティブタブラベルが `#aaaaaa`（グレー）・10pxテキストで表示されており、コントラスト比が2.32:1だった。この問題が特に深刻なのはなぜですか？",
    choices: [
      "小さいフォントは基準値が高くなるため、2倍の問題になる",
      "テキストが小さいほど視認に必要なコントラストは高くなるのに、AAの基準4.5:1を大きく下回っており、弱視・老眼・屋外での利用で判読不能になる",
      "10pxフォントは画面上に表示されないため問題ない",
      "グレーテキストは装飾扱いなのでコントラスト基準の対象外",
    ],
    answer: 1,
    explanation:
      "文字が小さいほど視認にはコントラストが必要です。WCAG 1.4.3 では18pt（約24px）以上または14pt（約19px）以上の太字以外は4.5:1以上が必要です。10pxで2.32:1では晴天下の屋外や加齢による視力低下があるユーザーには読めません。ナビラベルは重要なUIで「装飾」ではありません（WCAG 1.4.3）。",
  },
  {
    id: "rw-30",
    themeId: "real-world",
    type: "multiple_choice",
    question:
      "青背景（`#1a6dcc`）のバー上で濃いグレー（`#222222`）テキストを使ったところ、コントラスト比が3.11:1だった。何が間違っていますか？",
    choices: [
      "背景が青なので白テキストよりグレーの方がおしゃれで問題ない",
      "背景色に対してテキスト色のコントラストを確認すべきで、濃い文字でも暗い背景に対しては比率が下がる。4.5:1未満はAA不合格",
      "コントラストは前景色と白の比較で計算する",
      "3:1以上あれば全テキストでAA合格",
    ],
    answer: 1,
    explanation:
      "コントラスト比は「前景色と背景色の輝度差」で計算します。#222222は暗い色ですが、#1a6dcc（中程度の青）も比較的暗いため両者の差が小さくなります。白テキスト（5.12:1）は合格しているのに対して、同じ背景に暗色テキストを使うと逆に見えにくくなるケースです（WCAG 1.4.3）。",
  },
]
